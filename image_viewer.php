...
$query = "SELECT id, user_id, filename, file_type, file_size, file_path FROM files WHERE file_type = 'image/png' or file_type = 'image/jpeg' AND user_id = $user_id";
if (!empty($search)) {
  // Add a condition to search for filenames containing the search keyword
  $query .= " AND (filename LIKE '%$search%' OR (file_type = 'image/jpeg' AND filename LIKE '%$search%') OR (file_type = 'image/png' AND filename LIKE '%$search%'))";
}
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css" integrity="sha512-xh6O/CkQoPOWDdYTDqeRdPCVd1SpvCA9XXcUnZS2FmJNp1coAFzvtCN9BmamE+4aHK8yyUHUSCcJHgXloTyT2A==" crossorigin="anonymous" referrerpolicy="no-referrer">
    ...
    <script>
  function zoomIn(imagePath, imageName) {
    // Create a container element for the zoomed image
...
<h2 style="text-align:center;font-size:30px;color:#f37702;">Image Viewer</h2>
<form id="search-form" action="image_viewer.php" method="GET">
...
<table style="margin-left:30rem;margin-top:3rem;width:60%;padding:0rem;border:#f37702 5px solid">
    ...
    <?php while ($fileRow = $filesResult->fetch_assoc()) { ?>
        <tr>
        <td>
  <a href="#" onmouseover="zoomOnHover('<?php echo $fileRow['file_path']; ?>', '<?php echo $fileRow['filename']; ?>')">
    <img id="zoom-image" src="<?php echo $fileRow['file_path']; ?>" alt="<?php echo $fileRow['filename']; ?>" style="max-width: 100px; max-height: 100px;margin-left:0rem;padding:0.1rem;align-items:center;display:block;margin-left:auto;margin-right: auto;width: 50%;" 
    onclick="zoomIn('<?php echo $fileRow['file_path']; ?>', '<?php echo $fileRow['filename']; ?>')"
     onmouseout="this.style.transform = 'scale(1)'"onmouseover="zoomOnHover('<?php echo $fileRow['file_path']; ?>', '<?php echo $fileRow['filename']; ?>')" >
  </a>
</td>
            <td><?php echo '<div style="text-align:center;">' . $fileRow['filename'] . "</div>"; ?></td>
            <td>
                <form method="POST" action="image_viewer.php">
